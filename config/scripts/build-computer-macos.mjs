...
import computerUseBuildIdentity from './computer-use-build-identity.cjs'

const { resolveComputerUseBuildIdentity } = computerUseBuildIdentity
...
const buildIdentity = resolveComputerUseBuildIdentity({
  kind: process.env.ORCA_MAC_RELEASE === '1' ? 'release' : 'development',
  bundleIdOverride: process.env.ORCA_COMPUTER_MACOS_BUNDLE_ID
})
const { bundleId, displayName } = buildIdentity
