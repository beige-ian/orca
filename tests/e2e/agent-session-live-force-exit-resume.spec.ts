...
{}, testInfo) => {
  ...
  try {
    ...
    // Startup cold-restore reuses the existing tab. It must not create a
    // duplicate tab or auto-submit the old prompt as a second agent launch.
...
{}, testInfo) => {
  ...
  try {
    ...
    // main-side hook cache, which is the crash path this test is intended to
    // prove.
