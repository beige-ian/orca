...
export function requestAgentStatusStartupSnapshot(): Promise<void> {
  ...
  // After the bridge has mounted once, a later request can safely be a no-op
  // while its owner is unmounted (for example during React StrictMode setup).
  if (hasRegisteredLoader) {
